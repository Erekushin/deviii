...
class WalletConnector {
  ...
  factory WalletConnector() {
    ...
    connector.on('connect', (event) {
      GlobalVariables.connectionType.value = true;
    });

    connector.on('session_update', (payload) {
      print('PayloadWallet: $payload');
    });

    connector.on('disconnectWallet', (event) {
      print('Disconnected');
      GlobalVariables.connectionType.value = false;
    });

    connector.registerListeners(onSessionUpdate: (payload) {
      print("WalletPayloadddd: $payload");
    });
  ...
  Future<String> signTransactions(SessionStatus session) async {
    print('manlauio: ${session.accounts[0]}');
    final sender = Address.fromAlgorandAddress(address: session.accounts[0]);
